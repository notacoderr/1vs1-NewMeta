...
class Arena{
	...
	private function giveKit(Player $player){
		...
		$player->addTitle("§l§a1vs1", "§r§fGodspeed and Goodluck");
   }
   ...
   public function onPlayerDeath(Player $loser) {
    ...
    public function resultForm(Player $player, int $xp,int $rp,int $key)
    {
		...
		$form->setTitle(" §l1vs1§r§7 Post battle result");
		...
		$s = "";
		$s .= "§b Experience points: +§7".$xp."§r\n";
		$s .= "§b Respect points: +§7". $rp. "§r\n";
		$s .= "§b Bonus: +§7". $key. "§f common crate keys§r\n";
		$s .= "§b Current ELO: §7".$rank." ".$div." §b| RP: §7[§c".$resp."§7] §f§r\n";
		$s .= "§r\n";
        $form->setContent($s);
